...
def G_0(n_network_g_0, max_n_g, big_M_g, O_k_g, D_s_g):
    tks_matrix = []
    for n in range(max_n_g):
        tks_matrix.append([])
        for i, ok in enumerate(O_k_g):
            for j in range(len(D_s_g)):
                if i != j and ok != 0:
                    tksn = shortest_path(origin= i, car_network_sp= n_network_g_0)[0][j]
...
def Reopening_prog(O_k, D_s, initial_network, big_M, max_n,
    ...
    while True:
        ...
        G_y0 = G_0(n_network_g_0= n_network[0], max_n_g= max_n,
            big_M_g= big_M, O_k_g= O_k, D_s_g= D_s)

        if G >= G_best:
            G_best = round(G)
            link_with_ant_best = [[j for j in i] for i in link_with_ant]
            delta_tau = (G_best - G_y0)/G_y0
    ...
    # print(ant_choose)
    # ant choose is an array of some arrays,
    # every internal array has 3 elements,
    # 1st. element is ant no., 2nd. is n (time),
    # 3rd. is the chosen damaged link for reopening by ant in n

    return(G_best, iteration, ant_choose)
